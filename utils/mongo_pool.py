...
class MongoPool(object):
    ...
    def find_sort(self, conditions, count=0):
        ...
        proxies = self.proxies.find(filter=conditions, limit=int(count)).sort(
    ...
    def add_disable_domain(self, ip, domain):
        count = self.proxies.count_documents({"_id": ip})
        if count == 1:
...
if __name__ == "__main__":
    ...
    proxies = mongo_pool.get_proxies(protocol=None, nick_type=0, domain=None, count=1)
    print(proxies)
